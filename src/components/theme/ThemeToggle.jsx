...
// // 1. Define your DaisyUI theme names as constants
// const LIGHT_THEME = "krishilinkLight";
// const DARK_THEME = "krishilinkDark";

// // Utility function to get the current theme from the HTML element
// const getInitialTheme = () => {
//   if (typeof document !== "undefined") {
//     const theme = document.documentElement.getAttribute("data-theme");
//     // Check if the current theme is explicitly the custom dark theme
//     return theme === DARK_THEME;
//   }
//   // Default to light if environment is not defined (SSR)
//   return false;
// };

// const ThemeToggle = () => {
//   // Initialize state based on the current HTML data-theme attribute
//   const [isDarkTheme, setIsDarkTheme] = useState(getInitialTheme);

//   // Effect to handle initial theme setting and prevent flicker
//   useEffect(() => {
//     const html = document.documentElement;
//     const currentTheme = html.getAttribute("data-theme");

//     // If the theme is not set, or is an unknown value, set it based on state
//     if (
//       !currentTheme ||
//       (currentTheme !== LIGHT_THEME && currentTheme !== DARK_THEME)
//     ) {
//       html.setAttribute("data-theme", isDarkTheme ? DARK_THEME : LIGHT_THEME);
//     }

//     // Update state to match the attribute in case the browser loaded with a default
//     setIsDarkTheme(html.getAttribute("data-theme") === DARK_THEME);
//   }, []); // Run only once on mount

//   // Function to toggle the theme on the HTML element and update state
//   const toggleTheme = () => {
//     const html = document.documentElement;
...
//   // Use DaisyUI classes for the toggle appearance
//   return (
//     <label className="themeSwitcherThree relative inline-flex cursor-pointer select-none items-center">
//       {/* Hidden checkbox input that drives the toggle action */}
//       <input
//         type="checkbox"
//         checked={isDarkTheme}
...
//       {/* The visible switch mechanism - uses bg-base-100 for theme support */}
//       <div className="shadow-lg flex h-[46px] w-[82px] items-center justify-center rounded-md bg-base-100">
//         {/* Light Mode Icon (Left Side - Active when NOT dark) */}
//         <span
//           className={`flex h-9 w-9 items-center justify-center rounded transition-colors duration-300 ${
//             !isDarkTheme
//               ? "bg-primary text-base-100" // Active: Primary background, Base-100 text
//               : "text-base-content opacity-70" // Inactive: Base-content text
//           }`}
//         >
...
//         {/* Dark Mode Icon (Right Side - Active when dark) */}
//         <span
//           className={`flex h-9 w-9 items-center justify-center rounded transition-colors duration-300 ${
//             isDarkTheme
//               ? "bg-primary text-base-100" // Active: Primary background, Base-100 text
...
const ThemeToggle = () => {
  // Use a state to track if the 'dark' theme (checked state) is active
  const [isDarkTheme, setIsDarkTheme] = useState(getInitialTheme);

  // Function to toggle the theme on the HTML element
  const toggleTheme = () => {
    const html = document.documentElement;
  ...
  // Optional: Set initial theme attribute on mount if not already set
  useEffect(() => {
    const html = document.documentElement;
  ...
  // I've kept the original structure and Tailwind classes from Switcher13
  return (
    <label className="themeSwitcherThree relative inline-flex cursor-pointer select-none items-center">
      {/* Hidden checkbox input that drives the toggle action */}
      <input
        type="checkbox"
        checked={isDarkTheme} // Use isDarkTheme state
        onChange={toggleTheme} // Use the combined toggleTheme function
        className="sr-only"
      />

      {/* Optional Text Label */}

      {/* The visible switch mechanism */}
      <div className="shadow-card flex h-[46px] w-[82px] items-center justify-center rounded-md bg-white dark:bg-gray-800">
        ...
        >
          {/* SVG for Sun/Light Mode (from Switcher13, slightly modified for color context) */}
          <svg
            width="16"
            height="16"
            viewBox="0 0 16 16"
        ...
        >
          {/* SVG for Moon/Dark Mode (from Switcher13) */}
          <svg
            width="16"
            height="16"
            viewBox="0 0 16 16"
